...
namespace amp
{
	...
	namespace detail
	{
		...
		inline bool geodesic_erode_32f_c1_iter(accelerator_view acc_view, array_view<const float, 2> src_array, array_view<float, 2> dest_array
			, array_view<const float, 2> mask_array, concurrency::array<int, 2>& gpu_mod_flag, MorphOp erode_op)
		{
			...
			parallel_for_each(acc_view, dest_array.get_extent().tile<tile_size, tile_size>().pad(), [=, &gpu_mod_flag](tiled_index<tile_size, tile_size> idx) restrict(amp)
			{
				...
				{
					float result = erode_op(data, local_row, local_col);
					float new_value = fast_math::fmaxf(result, mask_array(idx.global));
		...
		template<typename MorphOp>
		inline void geodesic_erode_32f_c1_iter(accelerator_view acc_view, array_view<const float, 2> src_array, array_view<float, 2> dest_array
			, array_view<const float, 2> mask_array, MorphOp erode_op)
		{
			static const int tile_size = 32;
			static const int tile_static_size_3x3 = 34;
			parallel_for_each(acc_view, dest_array.get_extent().tile<tile_size, tile_size>().pad(), [=](tiled_index<tile_size, tile_size> idx) restrict(amp)
			{
				...
				if (dest_array.get_extent().contains(idx.global))
				{
					dest_array(idx.global) = fast_math::fmaxf(erode_op(data, local_row, local_col), mask_array(idx.global));
				}
			});
		}
		...
		template<typename MorphOp>
		inline void geodesic_erode_32f_c1_iter(accelerator_view acc_view, array_view<const float, 2> src_array, array_view<float, 2> dest_array
			, array_view<const float, 2> mask_array, concurrency::array<int_2, 1>& seeds, array_view<int, 1> seeds_count, MorphOp erode_op)
		{
			static const int tile_size = 32;
			static const int tile_static_size_3x3 = 34;
			parallel_for_each(acc_view, dest_array.get_extent().tile<tile_size, tile_size>().pad(), [=, &seeds](tiled_index<tile_size, tile_size> idx) restrict(amp)
			{
				...
				if (dest_array.get_extent().contains(idx.global))
				{
					...
					float new_value = fast_math::fmaxf(erode_op(data, local_row, local_col), mask_array(idx.global));
					dest_array(idx.global) = new_value;
		...
		inline void geodesic_erode_32f_c1_connective4_iter(accelerator_view& acc_view, array_view<float, 2> work_array, array_view<const float, 2> mask_array
				...
				for (int i = 0; i < loop_count; i++)
				{
					if (seed.x >= 0)
					{
						if (seed.y > 0)
						{
							...
							float new_value = fast_math::fminf(old_value, guarded_read(work_array, concurrency::index<2>(seed.y - 2, seed.x), FLT_MAX));
							new_value = fast_math::fminf(new_value, guarded_read(work_array, concurrency::index<2>(seed.y, seed.x), FLT_MAX));
							new_value = fast_math::fminf(new_value, guarded_read(work_array, concurrency::index<2>(seed.y - 1, seed.x - 1), FLT_MAX));
							new_value = fast_math::fminf(new_value, guarded_read(work_array, concurrency::index<2>(seed.y - 1, seed.x + 1), FLT_MAX));
							new_value = fast_math::fmaxf(new_value, mask_array(seed.y - 1, seed.x));
						...
						{
							float old_value = work_array(seed.y + 1, seed.x);
							float new_value = fast_math::fminf(old_value, guarded_read(work_array, concurrency::index<2>(seed.y, seed.x), FLT_MAX));
							new_value = fast_math::fminf(new_value, guarded_read(work_array, concurrency::index<2>(seed.y + 2, seed.x), FLT_MAX));
							new_value = fast_math::fminf(new_value, guarded_read(work_array, concurrency::index<2>(seed.y + 1, seed.x - 1), FLT_MAX));
							new_value = fast_math::fminf(new_value, guarded_read(work_array, concurrency::index<2>(seed.y + 1, seed.x + 1), FLT_MAX));
							new_value = fast_math::fmaxf(new_value, mask_array(seed.y + 1, seed.x));
							if (new_value != old_value)
						...
						if (seed.x > 0)
						{
							...
							float new_value = fast_math::fminf(old_value, guarded_read(work_array, concurrency::index<2>(seed.y - 1, seed.x - 1), FLT_MAX));
							new_value = fast_math::fminf(new_value, guarded_read(work_array, concurrency::index<2>(seed.y + 1, seed.x - 1), FLT_MAX));
							new_value = fast_math::fminf(new_value, guarded_read(work_array, concurrency::index<2>(seed.y, seed.x - 2), FLT_MAX));
							new_value = fast_math::fminf(new_value, guarded_read(work_array, concurrency::index<2>(seed.y, seed.x), FLT_MAX));
							new_value = fast_math::fmaxf(new_value, mask_array(seed.y, seed.x - 1));
							if (new_value != old_value)
							{
		...
		inline void geodesic_erode_32f_c1_connective8_iter(accelerator_view& acc_view, array_view<float, 2> work_array, array_view<const float, 2> mask_array
			, const concurrency::array<int_2, 1>& seeds, int cpu_seeds_count
			, concurrency::array<int_2, 1>& new_seeds, array_view<int, 1> new_seeds_count)
		{
			static const int tile_size = 64;
				...
				for (int i = 0; i < loop_count; i++)
				{
					if (seed.x >= 0)
					{
						if (seed.y > 0)
						{
							...
							new_value = fast_math::fminf(new_value, guarded_read(work_array, concurrency::index<2>(seed.y - 1, seed.x - 1), FLT_MAX));
							new_value = fast_math::fminf(new_value, guarded_read(work_array, concurrency::index<2>(seed.y - 1, seed.x + 1), FLT_MAX));
							new_value = fast_math::fminf(new_value, guarded_read(work_array, concurrency::index<2>(seed.y - 2, seed.x - 1), FLT_MAX));
							new_value = fast_math::fminf(new_value, guarded_read(work_array, concurrency::index<2>(seed.y - 2, seed.x + 1), FLT_MAX));
							new_value = fast_math::fminf(new_value, guarded_read(work_array, concurrency::index<2>(seed.y, seed.x - 1), FLT_MAX));
							new_value = fast_math::fminf(new_value, guarded_read(work_array, concurrency::index<2>(seed.y, seed.x + 1), FLT_MAX));
							new_value = fast_math::fmaxf(new_value, mask_array(seed.y - 1, seed.x));
							if (new_value != old_value)
							{
						...
						if (seed.x > 0)
						{
							...
							new_value = fast_math::fminf(new_value, guarded_read(work_array, concurrency::index<2>(seed.y, seed.x - 2), FLT_MAX));
							new_value = fast_math::fminf(new_value, guarded_read(work_array, concurrency::index<2>(seed.y, seed.x), FLT_MAX));
							new_value = fast_math::fminf(new_value, guarded_read(work_array, concurrency::index<2>(seed.y + 1, seed.x - 2), FLT_MAX));
							new_value = fast_math::fminf(new_value, guarded_read(work_array, concurrency::index<2>(seed.y + 1, seed.x), FLT_MAX));
							new_value = fast_math::fminf(new_value, guarded_read(work_array, concurrency::index<2>(seed.y - 1, seed.x - 2), FLT_MAX));
							new_value = fast_math::fminf(new_value, guarded_read(work_array, concurrency::index<2>(seed.y - 1, seed.x), FLT_MAX));
							new_value = fast_math::fmaxf(new_value, mask_array(seed.y, seed.x - 1));
							if (new_value != old_value)
	...
	template<typename MorphOp>
	inline void geodesic_erode_32f_c1(accelerator_view acc_view, array_view<float, 2> dest_array, array_view<const float, 2> mask_array
		, array_view<float, 2> temp_array, int max_iter)
	{
		...
		if (max_iter == 0)
		{
			...
			seeds_count1[0] = 0;
			seeds_count2[0] = 0;
			detail::geodesic_erode_32f_c1_iter(acc_view, dest_array, temp_array, mask_array, erode_op);
			detail::geodesic_erode_32f_c1_iter(acc_view, temp_array, dest_array, mask_array, seeds1, seeds_count1, erode_op);
			...
			if (cpu_seeds_count1 > 0)
			{
				if (MorphOp::connective == 4)
				{
					while (true)
					{
						detail::geodesic_erode_32f_c1_connective4_iter(acc_view, dest_array, mask_array, seeds1, cpu_seeds_count1, seeds2, seeds_count2);
						if ((cpu_seeds_count2 = seeds_count2[0]) == 0) break;
						detail::geodesic_erode_32f_c1_connective4_iter(acc_view, dest_array, mask_array, seeds2, cpu_seeds_count2, seeds1, seeds_count1);
				...
				else
				{
					while (true)
					{
						detail::geodesic_erode_32f_c1_connective8_iter(acc_view, dest_array, mask_array, seeds1, cpu_seeds_count1, seeds2, seeds_count2);
						if ((cpu_seeds_count2 = seeds_count2[0]) == 0) break;
						detail::geodesic_erode_32f_c1_connective8_iter(acc_view, dest_array, mask_array, seeds2, cpu_seeds_count2, seeds1, seeds_count1);
						if ((cpu_seeds_count1 = seeds_count1[0]) == 0) break;
					}
				}
			}
		...
		else
		{
			...
			while (true)
			{
				...
				if (!detail::geodesic_erode_32f_c1_iter(acc_view, temp_array, dest_array, mask_array, gpu_mod_flag, erode_op) || ++i == max_iter)
				{
					break;
				}
			}
	...
	template<typename MorphOp>
	inline void geodesic_erode_32f_c1(accelerator_view acc_view, array_view<const float, 2> src_array, array_view<float, 2> dest_array
		, array_view<const float, 2> mask_array, array_view<float, 2> temp_array, int max_iter)
	{
		...
		geodesic_erode_32f_c1<MorphOp>(acc_view, dest_array, mask_array, temp_array, max_iter);
	}
		...
		, array_view<const float, 2> mask_array, array_view<float, 2> temp_array, int max_iter = 0, bool connective_4 = false)
	{
		if (connective_4)
		{
			geodesic_erode_32f_c1<erode_4_connective>(acc_view, src_array, dest_array, mask_array, temp_array, max_iter);
		}
		else
		{
			geodesic_erode_32f_c1<erode_8_connective>(acc_view, src_array, dest_array, mask_array, temp_array, max_iter);
		}
	}
		...
		, array_view<float, 2> temp_array, int max_iter = 0, bool connective_4 = false)
	{
		if (connective_4)
		{
			geodesic_erode_32f_c1<erode_4_connective>(acc_view, dest_array, mask_array, temp_array, max_iter);
		}
		else
		{
			geodesic_erode_32f_c1<erode_8_connective>(acc_view, dest_array, mask_array, temp_array, max_iter);
		}
	}
}
